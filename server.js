...
// Verify a recovery code and update the password in the database. The code is
// the one this server generated and stored (hashed) on the user row in
// /api/send-reset-email — never anything the client supplied. A client-chosen
// code would let anyone name a code for someone else's account, ask us to
// remember it, and immediately redeem it.
...
app.post('/api/auth/reset-password', emailLimiter, async (req, res) => {
  ...
  try {
    ...
    const expired = !row.reset_code_expires_at || new Date(row.reset_code_expires_at) <= new Date()
    if (!row.reset_code_hash || expired) {
      return res.status(401).json({ error: 'Invalid or expired recovery code.' })
    }
    // A 6-digit code is small enough to guess through; cap the tries on the
    // account itself, since emailLimiter is per-IP and rotatable.
    if ((row.reset_code_attempts || 0) >= RESET_CODE_MAX_ATTEMPTS) {
      return res.status(429).json({ error: 'Too many incorrect codes. Request a new one.' })
    }

    const codeOk = await verifyPassword(code.trim(), row.reset_code_hash)
    if (!codeOk) {
      await query(
        'UPDATE users SET reset_code_attempts = reset_code_attempts + 1 WHERE id = $1',
        [row.id],
      ).catch((e) => console.error('reset attempt count failed:', e.message))
      return res.status(401).json({ error: 'Invalid or expired recovery code.' })
    }

    const hash = await hashPassword(newPassword)
    // Single use: the code is burned whether or not the user asks for another.
    await query(
      `UPDATE users
          SET password_hash = $1, reset_code_hash = NULL,
              reset_code_expires_at = NULL, reset_code_attempts = 0
        WHERE id = $2`,
      [hash, row.id],
    )
...
app.post('/api/send-reset-email', emailLimiter, async (req, res) => {
  ...
  // For a server-backed account the code is minted here and stored hashed on
  // the row — req.body.code is ignored outright, because a caller-chosen code
  // is a caller-chosen credential. PIN recovery and accounts that exist only
  // in the browser's localStorage have no server-side secret to protect, so
  // there the client's code is still just relayed into the email.
  let code = typeof req.body.code === 'string' ? req.body.code : ''
  let serverGenerated = false
  const normalizedEmail = email.trim().toLowerCase()

  if (type === 'password' && hasDatabase()) {
    try {
  ...
  try {
    ...
    // `serverGenerated` tells the client its own locally-generated code is not
    // the one that was emailed, so it must not show it on screen.
    return res.status(200).json({ ok: true, serverGenerated })
  } catch (error) {
    console.error('Email send failed:', error)
    return res.status(500).json({ error: 'Failed to send email.', serverGenerated })
  }
})
