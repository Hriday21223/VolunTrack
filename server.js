...
// Password-reset codes are minted here, never by the caller. Only a bcrypt
// hash of the code is stored (users.reset_code_hash), so neither a database
// read nor anything in the request body can be turned into a valid code.
...
// Step 1 of a password reset: the server mints the code, stores only its
// hash, and emails it. The response is deliberately identical whether or not
// the address has an account, so this can't be used to enumerate users.
app.post('/api/auth/request-password-reset', emailLimiter, async (req, res) => {
  ...
  try {
    ...
    if (!transport) {
      // Without SMTP there is no way to prove the requester owns the address,
      // so the code may only ever be handed back outside production — that is
      // the local/demo path, and in production it stays an error.
      if (isProd) {
        await clearResetCode(rows[0].id)
        return res.status(503).json({ error: 'Email backend is not configured.', missingVars: missing })
      }
      return res.json({ ok: true, emailed: false, code, missingVars: missing })
    }

    await transport.sendMail({
      from: process.env.EMAIL_FROM || process.env.EMAIL_USER,
      to: email,
      subject: 'VolunTrack password recovery code',
      text: `Your VolunTrack recovery code is ${code}. It expires in 15 minutes. ${emailFooterText()}`,
      html: `<p>Your VolunTrack recovery code is <strong>${code}</strong>.</p><p>It expires in 15 minutes.</p>${emailFooterHtml()}`,
    })
    return res.json({ ok: true, emailed: true })
  } catch (error) {
    console.error('request-password-reset failed:', error)
    return res.status(500).json({ error: 'Could not send a recovery code.' })
  }
})

// Step 2: verify the emailed code and set the new password. Every failure
// answers with the same message so neither the code nor the existence of the
// account leaks, and attempts are capped so the 6-digit space can't be walked.
app.post('/api/auth/reset-password', emailLimiter, async (req, res) => {
  ...
  const rejection = { error: 'Invalid or expired recovery code.' }

  try {
    const { rows } = await query(
      'SELECT id, reset_code_hash, reset_code_expires_at, reset_code_attempts FROM users WHERE email = $1',
      [email],
    )
    const row = rows[0]
    if (!row || !row.reset_code_hash || !row.reset_code_expires_at) {
      return res.status(401).json(rejection)
    }

    const expired = new Date(row.reset_code_expires_at).getTime() <= Date.now()
    if (expired || row.reset_code_attempts >= RESET_CODE_MAX_ATTEMPTS) {
      await clearResetCode(row.id)
      return res.status(401).json(rejection)
    }

    const ok = await verifyPassword(code.trim(), row.reset_code_hash)
    if (!ok) {
      await query('UPDATE users SET reset_code_attempts = reset_code_attempts + 1 WHERE id = $1', [row.id])
      return res.status(401).json(rejection)
    }
...
// Deliver a PIN recovery code. The PIN is a local-only credential (its hash
// never leaves the browser), so the code is generated client-side and this
// endpoint only relays it. Password codes must not come through here — they
// are server-minted by /api/auth/request-password-reset above.
app.post('/api/send-reset-email', emailLimiter, async (req, res) => {
  ...
  try {
    await transport.sendMail({
      from: process.env.EMAIL_FROM || process.env.EMAIL_USER,
      to: email,
      subject: 'VolunTrack PIN recovery code',
      text,
      html,
    })
